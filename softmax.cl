...
__kernel void max(__global const float *arr, __global const size_t *dims, __global float *results, int numDims, int axis) {
    int id = get_global_id(0);
...
__kernel void sum(__global const float *arr, __global const size_t *dims, __global float *results, int numDims, int axis) {
    int id = get_global_id(0);
...
__kernel void sub_and_exp(__global float *arr, __global const size_t *dims,
    ...
    for (int i = 0; i < numDims; i++) {
        if (valDims[i] != 1) {
            valPos += ( (i / dimCntr[i]) % dims[i] ) * valDimsCntr[i];
        }
    }
